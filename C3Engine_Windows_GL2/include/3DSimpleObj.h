...
public:
	...
	// 永久改变当前实例对象的贴图
	// idTex/idMixTex为配置在3DTexture.ini里的ID.
	...
	// 仅根据传入矩阵进行渲染,忽略所有SetPos/SetScale/SetDeep/SetRotateX/SetRotate/SetMirror的位置设置.
	// Note: 使用Draw3D来镜像物体的操作(即将传入的matInplace矩阵的_11成员变量乘以-1)需要设置SetMirror(true),以通知底层处理正确的三角形顺序.
